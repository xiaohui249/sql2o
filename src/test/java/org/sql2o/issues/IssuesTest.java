...
public class IssuesTest {
    ...
    /**
     * Tests for issue #1 https://github.com/aaberg/org.sql2o/issues/1
     *
     * Issue:
     * I have a case where I need to override/modify the value loaded from db.
     * I want to do this in a setter but the current version of org.sql2o modifies the property directly.
     *
     * Comment:
    ...
    /**
     *  Tests for issue #2 https://github.com/aaberg/org.sql2o/issues/2
     *
    ...
    /**
     *  Tests for issue #3 https://github.com/aaberg/org.sql2o/issues/3
     *
    ...
    /**
     *  Tests for issue #4 https://github.com/aaberg/org.sql2o/issues/4
     *
     *  NPE when typing wrong column name in row.get(...)
     *  Also, column name should not be case sensitive, if org.sql2o not is in casesensitive property is false.
     */
    @Test public void testForNpeInRowGet(){
        ...
        try{
            String row1Value = row1.getString("ahsHashah"); // Should fail with an org.sql2o exception
        }
        catch(Sql2oException ex){
            failed = true;
    ...
    /**
     *  Tests for issue #5 https://github.com/aaberg/org.sql2o/issues/5
    ...
    /**
     * Tests for issue #9 https://github.com/aaberg/org.sql2o/issues/9
     * When running a select query with column labels (aliases) in HSQLDB, org.sql2o is still trying to use column names
     * wheWhen running a select query with column labels (aliases) in HSQLDB, org.sql2o is still trying to use column names when mapping to java classes. This is caused by a behavior in HSQLDB, that is different from most other databases. the ResultSet.getColumnName() method will still return the real column name, even though a label was used. To get the label with HSQLDB, ResultSet.getColumnLabel().n mapping to java classes. This is caused by a behavior in HSQLDB, that is different from most other databases.
